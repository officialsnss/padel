                        ...
                        <tbody id="tablecontentt">
                            ...
                            @foreach ($courtUsers as $courtUser)
                                <tr class="row1" data-id="{{ $courtUser->cid }}">
                                    ...
                                    <td><input type="checkbox" data-id="{{ $courtUser->cid }}" name="popular"
                                            class="js-switchs" {{ $courtUser->isPopular == 1 ? 'checked' : '' }}></td>
                                    <td><input type="checkbox" data-id="{{ $courtUser->id }}" name="status"
                                            class="js-switch" {{ $courtUser->status == 1 ? 'checked' : '' }}></td>

                                    <td>
                                        <a href="{{ route('court-owners.view', $courtUser->id) }}"
                                            class="btn btn-info">View</a>
                                        <a href="{{ route('club.edit', $courtUser->cid) }}"
                                            class="btn btn-secondary">Edit</a>
                                        <a href="{{ route('club.images', $courtUser->cid) }}"
                                            class="btn btn-warning">Upload Images</a>&nbsp;
                                        <a href="{{ route('court-owners.resetPassword', $courtUser->id) }}"
